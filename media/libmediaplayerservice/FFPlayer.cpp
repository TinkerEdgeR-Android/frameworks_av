...
typedef int (*CreateFFPlayer)(FF_MediaPlayer **mediaPlayer);
typedef int (*DestroyFFPlayer)(FF_MediaPlayer **mediaPlayer);

FFPlayer::FFPlayer()
    : mPlayer(NULL) {
    ALOGV("FFPlayer");
    createPlayer(LIB_FFPLAYER);
...
void FFPlayer::createPlayer(const char *libname) {
    ...
    CreateFFPlayer createFFPlayer = (CreateFFPlayer)dlsym(mVendorLibHandle, "player_create");
    if (createFFPlayer) {
        createFFPlayer(&mPlayer);
    } else {
        ALOGE("Not found create player method)");
    }
}

void FFPlayer::destroyPlayer() {
    if (mVendorLibHandle == NULL) {
        if (mPlayer) {
            delete mPlayer;
            mPlayer = NULL;
    ...
    DestroyFFPlayer destroyFFPlayer = (DestroyFFPlayer)dlsym(mVendorLibHandle, "player_destroy");
    if (destroyFFPlayer) {
        destroyFFPlayer(&mPlayer);
    } else if (mPlayer) {
        delete mPlayer;
        mPlayer = NULL;
    }
...
status_t FFPlayer::seekTo(int msec, MediaPlayerSeekMode mode) {
    ALOGV("seekTo %.2f secs", msec / 1E3);

    status_t err = mPlayer->seekTo((int64_t)msec * 1000, mode);
